...
contract MixinWrapperFunctions is ReentrancyGuard, LibMath, LibFillResults, LibAbiEncoder, MExchangeCore, MWrapperFunctions {
    ...
    function fillOrderNoThrow(
        ...
        assembly {
            ...
            if success {
                ...
                mstore(
                    add(fillResults, 32),
                    mload(add(fillOrderCalldata, 32))
                )
                mstore(
                    add(fillResults, 64),
                    mload(add(fillOrderCalldata, 64))
                )
                mstore(
                    add(fillResults, 96),
                    mload(add(fillOrderCalldata, 96))
                )
            }
        }
    ...
    function batchFillOrdersNoThrow(
        ...
        uint256[] memory takerAssetFillAmounts,
        bytes[] memory signatures
    ) public returns (FillResults memory totalFillResults) {
        uint256 ordersLength = orders.length;
        for (uint256 i = 0; i != ordersLength; i++) {
        ...
        returns (FillResults memory totalFillResults)
    {
        ...
        for (uint256 i = 0; i != ordersLength; i++) {
            ...
            uint256 remainingTakerAssetFillAmount = safeSub(
                takerAssetFillAmount,
        ...
        returns (FillResults memory totalFillResults)
    {
        ...
        for (uint256 i = 0; i != ordersLength; i++) {
            ...
            uint256 remainingMakerAssetFillAmount = safeSub(
    ...
    {
        uint256 ordersLength = orders.length;
        LibOrder.OrderInfo[] memory ordersInfo = new LibOrder.OrderInfo[](
            ordersLength
        );
        for (uint256 i = 0; i != ordersLength; i++) {
